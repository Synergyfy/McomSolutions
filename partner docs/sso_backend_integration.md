...
| `state` | ✅ | `<random-csrf-token>` | Random string generated by your frontend — returned verbatim |
...
3. Checks if the user has an active `mcom_session` cookie
   ...
   - If **no** → redirects user to MCOM Central's login page, then back here after login
