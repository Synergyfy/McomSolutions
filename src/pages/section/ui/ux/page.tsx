...
// ═══════════════════════════════════════════════════════════
// Mock API Hooks, Redux, and Services (Simulated Backend)
// ═══════════════════════════════════════════════════════════
const UserRole = {
...
function useCreateUser() {
  return {
    mutateAsync: async (data: any) => {
      console.log('Mock: Creating business account', data);
      return {
        data: {
          token: 'mock-jwt-token',
          user: { id: 'user-id-123', email: data.email },
          auth: { accessToken: 'mock-access', refreshToken: 'mock-refresh' },
          listing: { id: 'listing-id-123' },
        }
      } as any;
    },
    isPending: false,
...
function useLogin() {
  return {
    mutateAsync: async (data: any) => {
      console.log('Mock: Login business account', data);
      return {
        data: {
          token: 'mock-jwt-token',
          user: { id: 'user-id-123', email: data.email },
...
function useSendOtp() {
  return {
    mutateAsync: async (data: any) => {
      console.log('Mock: Sending OTP to email', data);
      return { success: true };
    },
    isPending: false,
  };
...
function useAddListing() {
  return {
    mutateAsync: async (data: any) => {
      console.log('Mock: Adding listing', data);
...
const useDispatch = () => (action: any) => console.log('Mock Dispatch:', action);
...
const Cookies = {
  set: (key: string, value: string, options?: any) => console.log(`Mock Cookie Set: ${key} = ${value}`),
  get: (key: string) => null,
};

const api = {
  post: async (endpoint: string, payload: any): Promise<{ data: any }> => {
    console.log(`Mock API POST call to ${endpoint}`, payload);
    return {
      data: {
  ...
  get: async (endpoint: string): Promise<{ data: any }> => {
    console.log(`Mock API GET call to ${endpoint}`);
    // Return mock business results for google-business search endpoint
    if (endpoint.includes('google-business') && endpoint.includes('queryText')) {
      return {
        data: {
          results: [
            {
              ...
              vicinity: 'Camden High Street, London',
              rating: 4.5,
              ...
              branches: [
                { branchId: 'b001', name: 'Camden Main Branch', address: '14 Camden High Street, London, NW1 0JH', postcode: 'NW1 0JH', businessPhone: '020 7946 0001', googleCategoryId: 'gcid:coffee_shop' }
              ]
            },
            {
              ...
              vicinity: 'Chalk Farm Road, London',
              rating: 4.8,
              ...
              branches: [
                { branchId: 'b002', name: 'Chalk Farm Branch', address: '8 Chalk Farm Road, London, NW1 8AG', postcode: 'NW1 8AG', businessPhone: '020 7946 0022', googleCategoryId: 'gcid:bakery' }
              ]
            },
            {
              ...
              branches: [
                { branchId: 'b003', name: 'Parkway Branch', address: '52 Parkway, Camden, London, NW1 7AH', postcode: 'NW1 7AH', businessPhone: '020 7946 0033', googleCategoryId: 'gcid:clothing_store' }
              ]
            }
          ]
        }
    ...
    // Mock place details for individual business fetch
    if (endpoint.includes('google/google-business/mock-place-')) {
      const mockDetails: Record<string, any> = {
        'mock-place-001': {
          name: 'The Coffee Lounge',
          formatted_address: '14 Camden High Street, London, NW1 0JH',
          international_phone_number: '+44 20 7946 0001',
          website: 'https://thecoffeelounge.co.uk',
          rating: 4.5,
          user_ratings_total: 312,
          opening_hours: { open_now: true, weekday_text: ['Monday: 7:00 AM – 8:00 PM'] },
          types: ['coffee_shop', 'establishment'],
        },
        'mock-place-002': {
          name: 'Artisan Bakehouse',
          formatted_address: '8 Chalk Farm Road, London, NW1 8AG',
          international_phone_number: '+44 20 7946 0022',
          website: 'https://artisanbakehouse.co.uk',
          rating: 4.8,
          user_ratings_total: 189,
          opening_hours: { open_now: true, weekday_text: ['Monday: 6:00 AM – 6:00 PM'] },
          types: ['bakery', 'establishment'],
        },
        'mock-place-003': {
          name: 'Urban Style Boutique',
          formatted_address: '52 Parkway, Camden, London, NW1 7AH',
          international_phone_number: '+44 20 7946 0033',
          website: 'https://urbanstyleboutique.co.uk',
          rating: 4.3,
          user_ratings_total: 97,
          opening_hours: { open_now: false, weekday_text: ['Monday: 10:00 AM – 7:00 PM'] },
          types: ['clothing_store', 'establishment'],
        },
      };
      const placeIdKey = Object.keys(mockDetails).find(k => endpoint.includes(k));
      const detail = placeIdKey ? mockDetails[placeIdKey] : mockDetails['mock-place-001'];
      return { data: { result: detail } };
    }
    // Mock map-category endpoint
    if (endpoint.includes('google-business/map-category')) {
      return {
        data: {
          sectorId: 'sector-food',
          categoryId: 'cat-cafe',
