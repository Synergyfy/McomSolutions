    ...
    if (step === 7) {
      ...
      try {
        await businessApi.register({
          ...
          isOnGoogle: !!formData.isOnGoogle,
          ...
          {step !== 2 && step !== 8 && (
            <div className="mt-10 flex justify-end pt-6 border-t border-gray-100">
              <button 
                type="submit" 
                disabled={loading}
                className="flex items-center px-8 py-4 bg-orange-500 hover:bg-orange-600 text-white rounded-full font-bold transition-colors shadow-lg shadow-orange-500/20 disabled:opacity-50"
              >
                {step === 7 ? (loading ? 'Registering...' : 'Complete Registration') : 'Continue'}
