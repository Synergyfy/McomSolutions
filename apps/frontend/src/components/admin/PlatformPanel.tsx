...
import { ToggleLeft, Grid, Rocket, X, Edit3, Wrench, Eye, AlertTriangle, CheckCircle2 } from 'lucide-react';
import { cn } from '../../lib/utils';
import { useAdminData, Platform, PlatformLaunchRule } from '../../context/AdminDataContext';
...
export default function PlatformPanel() {
  ...
  return (
    <div>
      ...
      {tab === 'access' && <PlatformAccessPanel />}
      {tab === 'directory' && <PlatformDirectoryPanel />}
      {tab === 'launch' && <PlatformLaunchPanel />}
    </div>
  );
}

function PlatformAccessPanel() {
  const { platforms, updatePlatform } = useAdminData();

  const statusColor: Record<string, string> = { Enabled: 'bg-green-500', Disabled: 'bg-red-500', Maintenance: 'bg-amber-500', Restricted: 'bg-orange-500' };
      ...
      {platforms.map(p => (
        <div key={p.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          ...
          <div className="flex items-center justify-between text-xs text-gray-400 mb-4">
            <span className="font-bold">{p.totalUsers.toLocaleString()} users</span>
            <span>Launched {p.launchDate}</span>
          </div>
          <div className="flex gap-2">
            {(['Enabled', 'Disabled', 'Maintenance'] as const).map(status => (
              <button key={status} onClick={() => updatePlatform(p.id, { status })} className={cn("flex-1 py-2 rounded-xl text-[10px] font-bold transition-all", p.status === status ? "bg-brand-blue text-white" : "bg-gray-50 text-gray-400 hover:bg-gray-100")}>{status}</button>
            ))}
          </div>
        </div>
      ))}
...
function PlatformDirectoryPanel() {
  const { platforms, updatePlatform } = useAdminData();
  return (
...
function PlatformLaunchPanel() {
  const { launchRules, platforms, addLaunchRule, updateLaunchRule, deleteLaunchRule } = useAdminData();
        ...
        {launchRules.map(rule => (
          <div key={rule.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            <div className="flex items-start justify-between mb-3">
              <div><h3 className="font-bold text-gray-900">{platforms.find(p => p.id === rule.platformId)?.name || rule.platformId}</h3><p className="text-xs text-gray-500">Conditions: {rule.launchConditions}</p></div>
              <div className="flex gap-1.5">
                <button onClick={() => deleteLaunchRule(rule.id)} className="p-1.5 bg-gray-50 rounded-lg hover:bg-red-50 transition-all"><X className="w-3.5 h-3.5 text-gray-400" /></button>
              </div>
            </div>
        ...
        {showAdd && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/20 backdrop-blur-sm">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white rounded-3xl w-full max-w-md overflow-hidden shadow-2xl">
              ...
              <LaunchRuleForm onSave={(data) => { addLaunchRule(data); setShowAdd(false); }} onCancel={() => setShowAdd(false)} />
            </motion.div>
          </div>
        )}
      </AnimatePresence>
